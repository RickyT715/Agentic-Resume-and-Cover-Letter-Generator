...
class DeepSeekClient(AIClientBase):
    def __init__(self):
        ...
        logger.info("DeepSeekClient initialized (settings read dynamically per request)")

    def _get_client(self):
        """Get or create the OpenAI client pointing at DeepSeek."""
        try:
            import openai
        except ImportError:
        ...
        api_key = (
            self.settings_manager.get("deepseek_api_key")
            or getattr(app_settings, "deepseek_api_key", "")
            or "not-needed"
        )
        ...
        if self._client is None or self._current_api_key != api_key:
            ...
            logger.info("Created new DeepSeek client")
    ...
    def model(self) -> str:
        return (
            self.settings_manager.get("deepseek_model")
            or getattr(app_settings, "deepseek_model", "")
            or "deepseek-chat"
        )

    @property
    def _temperature(self) -> float | None:
        return self.settings_manager.get("deepseek_temperature")

    @property
    def _max_output_tokens(self) -> int | None:
        return self.settings_manager.get("deepseek_max_output_tokens")
