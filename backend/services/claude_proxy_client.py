...
class ClaudeProxyClient(AIClientBase):
    ...
    def _get_client(self):
        ...
        api_key = self.settings_manager.get("claude_proxy_api_key") or getattr(
            app_settings, "claude_proxy_api_key", ""
        ) or "not-needed"
    ...
    def model(self) -> str:
        return (
            self.settings_manager.get("claude_proxy_model")
    ...
    async def generate(
        ...
        **kwargs,
        ...
        logger.info(
            f"Sending request to Claude Proxy (model={self.model}, streaming=true)"
        )
